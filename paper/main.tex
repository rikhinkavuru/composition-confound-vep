...
\author{\Name{Rikhin Kavuru} \Email{rajeshkavuru@gmail.com}\\
...
AUROC $0.72$ on noncoding ClinVar and recovers $45$--$53\%$ of the above-chance AUROC of
...
On ClinVar the composition-only classifier reaches AUROC $0.720$ (bootstrap CI
$0.716$--$0.722$) with no sequence context and no model. The leading models reach $0.958$
...
matched benchmarks. Matching removes about half of the confound, which is a positive result
for TraitGym, but a measurable residual leaks past even MAF- and LD-matched controls.
...
\begin{table}[t]
\centering
\caption{Leave-one-chromosome-out AUROC, AUPRC, composition recovery ratio, and
Benjamini--Hochberg-adjusted DeLong $p$ for the composition baseline versus each model. The
recovery ratio is $(\text{AUROC}_{\text{comp}}-0.5)/(\text{AUROC}_{\text{model}}-0.5)$;
values above one mean the model does not exceed composition. $p$ tests whether the model
differs from the composition baseline.}
\label{tab:main}
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
\multicolumn{5}{l}{\emph{ClinVar (unmatched)}}\\
\quad Composition-only & 0.720 & 0.362 & -- & -- \\
...
\quad phyloP & 0.912 & 0.775 & 0.53 & $<10^{-16}$ \\
\addlinespace
\multicolumn{5}{l}{\emph{TraitGym Mendelian (matched)}}\\
\quad Composition-only & 0.586 & 0.153 & -- & -- \\
\quad GPN-MSA & 0.904 & 0.694 & 0.21 & $10^{-68}$ \\
\quad Evo2-7B & 0.710 & 0.385 & 0.41 & $10^{-8}$ \\
\quad Evo2-40B & 0.851 & 0.557 & 0.25 & $10^{-45}$ \\
\quad NT-2.5B & 0.534 & 0.120 & $>$1 & 0.03 \\
\quad Caduceus & 0.509 & 0.108 & $>$1 & $10^{-3}$ \\
\quad CADD & 0.893 & 0.712 & 0.22 & $10^{-51}$ \\
\quad phyloP & 0.857 & 0.655 & 0.24 & $10^{-34}$ \\
\addlinespace
\multicolumn{5}{l}{\emph{TraitGym complex (matched)}}\\
\quad Composition-only & 0.520 & 0.115 & -- & -- \\
\quad GPN-MSA & 0.580 & 0.212 & 0.25 & $10^{-5}$ \\
\quad Evo2-40B & 0.516 & 0.137 & 1.26 & 0.70 \\
\quad NT-2.5B & 0.516 & 0.114 & $>$1 & 0.70 \\
\quad Caduceus & 0.518 & 0.114 & $>$1 & 0.77 \\
\quad CADD & 0.616 & 0.250 & 0.18 & $10^{-13}$ \\
\quad phyloP & 0.583 & 0.184 & 0.24 & $10^{-6}$ \\
\addlinespace
\multicolumn{5}{l}{\emph{GLRB eQTL}}\\
\quad Composition-only & 0.596 & 0.633 & -- & -- \\
\quad GPN-MSA & 0.560 & 0.599 & $>$1 & $10^{-3}$ \\
\bottomrule
\end{tabular}
\end{table}
