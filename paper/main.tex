...
the recovery ratio depends on classifier capacity, which we fix in advance; a supervised
one-hot CNN trained on the same windows stays below both the composition baseline and the
gLMs on every benchmark (AUROC $0.52$--$0.56$), so the composition features are not merely a
strong learner exploiting labels. The ClinVar noncoding set includes cCRE variants that also
overlap coding annotations, but the effect is unchanged on the strict noncoding subset
(Section~\ref{sec:e2}). Our two clinical/statistical benchmarks span two control regimes,
ClinVar-benign for ClinVar and gnomAD-common frequency-matched controls for TraitGym, and the
composition recovery is largest exactly where the controls are themselves ascertainment-biased
(ClinVar-benign); a same-positive-set swap of ClinVar controls to gnomAD-common is a clean
follow-up. Composition and function are genuinely correlated, so the goal is not to declare
composition non-biological but to test whether a model advertised as understanding regulatory
grammar is reducible to a mutation-rate detector on the benchmarks used to certify it.
