...
GPN-MSA adds only $+0.013$ (Evo2-40B: $+0.193$ versus $+0.057$; TraitGym Mendelian: $+0.306$
versus $+0.007$). The strong models thus subsume composition and carry large independent signal
...
(Section~\ref{sec:e2}). Our two benchmarks span two control regimes, ClinVar-benign and
gnomAD-common frequency-matched (TraitGym), and recovery is largest where the controls are
themselves ascertainment-biased; a same-positive gnomAD-common swap is a clean follow-up.
Composition and function are genuinely correlated, so the goal is not to call composition
non-biological but to test whether a model advertised as learning regulatory grammar reduces
to a mutation-rate detector on the benchmarks used to certify it.
...
\paragraph{Reproducibility.} All scores are authors' released per-variant values (TraitGym,
goodarzilab Evo2-ClinVar, songlab genome-wide GPN-MSA) or public annotations (CADD, phyloP,
phastCons, SpliceAI, ENCODE cCREs); composition features use the Carlson pentanucleotide table
and hg38. Code, the per-context neutral tables, and the released \texttt{calibrate\_llr}
transform are available at
