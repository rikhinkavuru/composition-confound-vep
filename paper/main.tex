...
model quality is not confounded by our own inference. On TraitGym this gives eight gLMs
(GPN-MSA, an alignment-based multiple-sequence-alignment (MSA) model; GPN, its
single-sequence counterpart; Evo2-7B and Evo2-40B; Nucleotide Transformer; Caduceus;
HyenaDNA; and SpeciesLM) plus CADD, phyloP and phastCons. On ClinVar we use precomputed Evo2-7B/40B and Nucleotide Transformer scores
...
context and no embedding. As a learned-from-scratch reference we also train a small one-hot
CNN on $\pm 100$ bp windows under the same folds (the ``does pretraining help'' control).
...
$0.586$ (Mendelian) and $0.520$ (complex), with recovery ratios of $0.21$--$0.25$ for the
leading models (GPN-MSA, Evo2-40B, CADD, phyloP); weaker gLMs such as Evo2-7B ($0.41$) and
SpeciesLM ($0.72$) recover higher precisely because they sit closer to the composition
baseline. On GLRB causal eQTL the baseline
...
GPN-MSA adds only $+0.013$ (Evo2-40B: $+0.193$ versus $+0.057$). The strong models thus
subsume composition and carry large independent signal
on top of it, whereas the smaller single-sequence gLMs do not exceed composition
(Table~\ref{tab:main}). The confound inflates the leaderboard \emph{impression} without making
the stronger models reducible to composition.
...
confound bites hardest on the smaller single-sequence gLMs, which do not exceed the baseline on
any benchmark, consistent with \citet{tang2025koo}. Alignment-based and large autoregressive
models keep signal that survives
composition matching and concentrates on motif-disrupting variants. The confound is an
across-set ascertainment phenomenon, not a local-context artifact.
